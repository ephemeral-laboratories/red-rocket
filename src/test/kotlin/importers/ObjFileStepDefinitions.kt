...
class ObjFileStepDefinitions(universe: Universe) : En {
    ...
    init {
        ...
        Then("{shape_var}.point1 = parser.vertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).point1).isEqualTo(parser.vertex(i))
        }
        Then("{shape_var}.point2 = parser.vertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).point2).isEqualTo(parser.vertex(i))
        }
        Then("{shape_var}.point3 = parser.vertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).point3).isEqualTo(parser.vertex(i))
        }
        Then("{shape_var}.texturePoint1 = parser.textureVertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).texturePoint1).isEqualTo(parser.textureVertex(i))
        }
        Then("{shape_var}.texturePoint2 = parser.textureVertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).texturePoint2).isEqualTo(parser.textureVertex(i))
        }
        Then("{shape_var}.texturePoint3 = parser.textureVertices[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as BaseTriangle).texturePoint3).isEqualTo(parser.textureVertex(i))
        }
        Then("{shape_var}.normal1 = parser.normals[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as SmoothTriangle).normal1).isEqualTo(parser.normal(i))
        }
        Then("{shape_var}.normal2 = parser.normals[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as SmoothTriangle).normal2).isEqualTo(parser.normal(i))
        }
        Then("{shape_var}.normal3 = parser.normals[{int}]") { sv: String, i: Int ->
            assertThat((universe.shapes[sv] as SmoothTriangle).normal3).isEqualTo(parser.normal(i))
        }

        Then("{shape_var} includes {string} from parser") { sv: String, string: String ->
            assertThat((universe.shapes[sv] as Group).children).contains(parser.namedGroup(string))
        }

        Then("{shape_var}.material.diffuse = {color}") { sv: String, e: Color ->
            assertThat(universe.shapes[sv]!!.material.diffuse).isCloseTo(e, epsilon)
        }
    }
}
